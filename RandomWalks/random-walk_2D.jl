...
begin
	t = (0:nsteps) .* dt
	plot(x, y, lab=false, xlab="x (μm)", ylab="y (μm)", ratio=1)
end

# ╔═╡ 23f188ae-67b3-4929-8276-daccfd73a74b
md"""
# Tasks
- introduce rotational diffusion (`rotational_diffusivity`)
- modify the average reorientation rate (`turn_rate`)
- use a different motility pattern (`motility`)
- add more bacteria (and make them transparent with `alpha=0.2` in the `plot` command)
"""
